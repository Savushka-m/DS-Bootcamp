...
class Tests:
    def ticker_and_revenue(ticker, field):
        assert ticker == "TSLA"
        assert field == "Total Revenue"
    
    def type_of_return():
        res = parcing("TSLA", "Total Revenue")
        assert type(res) is tuple

    def exceptions():
        with pytest.raises(Exception):
            parcing("some kind of trash", "more trash")
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        Tests.ticker_and_revenue(ticker, field)
        Tests.type_of_return()
        Tests.exceptions()